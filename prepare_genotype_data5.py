...
# Step 3: Assign synthetic SampleIDs (assuming each row is a unique sample)
data['SampleID'] = range(len(data))

# Step 4: Create wide format (pivot table)
# Use 'Risk Allele' as column names for SNPs
data['Genotype_វ'] = data.apply(encode_genotype, axis=1)
pivot_data = data.pivot_table(index='SampleID', columns='Risk Allele', values='Genotype', aggfunc='first').reset_index()

# Step 5: Separate features and target
X = pivot_data.drop(columns=['SampleID', 'disease_risk'])
y = pivot_data['disease_risk']

# Step 6: Replace -1 with NaN for imputation
X = X.replace(-1, np.nan)

# Step 7: Drop columns with all NaN values
...
# Step 8: Impute missing values using the mode
